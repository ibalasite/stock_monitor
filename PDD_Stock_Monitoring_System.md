...
### FR-21 財務資料三源平行備援（Multi-Provider Financial Data Redundancy）

**目標**：估值計算所需的財務資料（股利、EPS、資產負債、本益比、股價歷史）必須具備三個獨立來源，當主來源達到速率上限或暫時不可用時，系統可直接使用其他已快取的來源資料繼續計算，不中斷估值流程。

**三個獨立資料來源（依覆蓋範圍排序）**：

| 來源 | 角色 | 資料存取方式 |
|------|------|-------------|
| FinMind（P1） | 主來源 | 個股 API（JSON），免費 300 req/h，付費 600 req/h |
| MOPS + TWSE（P2） | 第二來源 | 一次取全市場資料（bulk fetch），需多次季度呼叫 |
| Goodinfo（P3） | 第三來源 | 個股網頁 scraping，限速 15 秒/req |

**業務規格**：

1. **三源同時執行，不互相等待**：每次請求財務資料時，三個來源同步觸發（不是失敗才嘗試下一個）。
2. **各來源獨立 SWR Cache**：每個來源維護各自的 SWR（Stale-While-Revalidate）快取，快取鍵為 `(來源名稱, 股票代號, 資料集)`。各來源快取互不影響。
3. **以最新快取資料為準**：三源同時執行後，取快取時間（`fetched_at`）最新者的資料返回給估值方法。
4. **快取未命中 + 來源暫時失敗**：該來源本次不寫快取，不影響其他來源繼續執行。
5. **三源全部失敗**：回傳 `None`，估值方法記錄 `SKIP_INSUFFICIENT_DATA`，不輸出無效估值。

**資料深度要求**：備援來源提供的資料深度必須與主來源相同（例如要求 10 年本益比歷史則備援也需提供 10 年），不可以短期替代品代替。

**驗收條件**：
1. FinMind 達到速率上限（HTTP 402）時，若 MOPS 或 Goodinfo 有有效快取，估值方法仍可繼續計算，不輸出 `SKIP_*`。
2. 三源均無快取且均失敗時，該股票該方法記錄 `SKIP_INSUFFICIENT_DATA`，不產生錯誤估值。
3. 每個來源的快取獨立存取，清除其中一個來源的快取不影響其他來源。
4. 全市場掃描完成後，成功取得財務資料的股票比例高於 FinMind 單源（因 MOPS/Goodinfo 可補充 FinMind 未快取部分）。
