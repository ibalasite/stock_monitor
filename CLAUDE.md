...
- 三源均回傳 `None` 或 raise `ProviderUnavailableError` → 估值方法記 `SKIP_INSUFFICIENT_DATA`
...
- **禁止** `GoodinfoAdapter` 在 cache **stale**（HIT 但過期）時阻塞等待新 scraping；stale 必須立即回傳舊值 + 背景刷新。**MISS**（DB 無資料）則必須同步 scraping，不可提前 raise（CR-FIN-04）
